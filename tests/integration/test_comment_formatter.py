...
class TestCommentFormatter:
    ...
    def test_format_pr_comment_contains_score(
        ...
        assert "72.5" in comment
        # Chinese: "评分" means "score"
        assert "评分" in comment

    def test_format_pr_comment_contains_issues(
        self, formatter: CommentFormatter, sample_result: AggregatedResult
    ) -> None:
        comment = formatter.format_pr_comment(sample_result, "owner/repo", 42)
        assert "SQL injection" in comment
        # The formatter renders issues with source/category, e.g. (rule/security)
    ...
    def test_format_pr_comment_contains_repo_and_pr(
        ...
        # Formatter renders file paths from issues and file count
        assert "src/api.py" in comment or "src/utils.py" in comment
        assert "文件" in comment  # Chinese: "files" appears in header

    def test_format_pr_comment_has_recommendation(
        self, formatter: CommentFormatter, sample_result: AggregatedResult
    ) -> None:
        comment = formatter.format_pr_comment(sample_result, "owner/repo", 1)
        # Chinese: "建议" means "recommendation", shown for critical issues
        assert "建议" in comment
        assert "严重" in comment  # Chinese: "critical" appears in recommendation

    def test_format_pr_comment_empty_issues(self, formatter: CommentFormatter) -> None:
        result = AggregatedResult(
            issues=[],
            scores=ReviewScores(
                overall=100.0, security=100.0, performance=100.0, maintainability=100.0
            ),
        ...
        # Chinese: "未发现问题" means "no issues found", "整洁" means "clean"
        assert "未发现问题" in comment or "整洁" in comment

    def test_format_inline_comments_payload(
        self, formatter: CommentFormatter, sample_result: AggregatedResult
    ) -> None:
    ...
    def test_inline_comments_critical_first(
        self, formatter: CommentFormatter, sample_result: AggregatedResult
    ) -> None:
        payload = formatter.format_inline_comments_payload(sample_result, max_comments=50)
        if len(payload) >= 2:
